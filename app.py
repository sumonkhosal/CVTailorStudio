...
def build_external_prompt_text_prompt(
    ...
    """
    Builds a high-value reusable master prompt for an external LLM.
    This prompt explicitly includes ATS diagnostics, accepted suggestions,
    and cover-letter inputs so the external AI can produce better results.
    """

    def format_suggestions_for_prompt(items: List[Dict[str, Any]]) -> str:
        if not items:
            return "No accepted suggestions were provided."
    ...
    def format_list_or_none(items: Any) -> str:
        if isinstance(items, list) and items:
            return ", ".join(str(x) for x in items)
        return "None"

    def format_structure_notes(items: Any) -> str:
        if isinstance(items, list) and items:
...
FOUND KEYWORDS
--------------------------------
{format_list_or_none(scores.get("found_keywords", []))}

MISSING KEYWORDS
--------------------------------
{format_list_or_none(scores.get("missing_keywords", []))}

STRUCTURE NOTES
--------------------------------
{format_structure_notes(scores.get("structure_notes", []))}
"""
    else:
        score_block = """
ATS-STYLE EVALUATION SUMMARY
--------------------------------
No ATS score summary was provided.
"""

    accepted_suggestions_block = format_suggestions_for_prompt(accepted_suggestions)

    system_prompt = (
        "You are an expert multilingual CV and cover letter editor, ATS optimization specialist, "
        "and application strategist. Your job is to produce a reusable master prompt for another LLM. "
        "That master prompt must be detailed, forceful, and highly practical. "
        "It must explicitly use ATS diagnostics, accepted suggestions, and user cover-letter inputs. "
        "Return plain text only. Do not return JSON."
    )

    user_prompt = f"""You are an expert multilingual CV and cover letter editor. Your task is to generate highly tailored application documents (CVs and cover letters) based on the provided inputs.

Your core mission is to:
1. Analyze and understand the Source CV and Source Job Description, even if they are in different languages.
2. Preserve factual integrity. Absolutely no invention of facts is allowed.
3. Use ATS-style diagnostics actively to improve relevance, structure, and wording truthfully.
4. Apply all accepted suggestions naturally and deliberately.
5. Produce final application documents in both English and German.

You are NOT starting from scratch.
You are receiving a pre-analyzed optimization package.
You must use it intelligently.

==================================================
NON-NEGOTIABLE RULES
==================================================

1. Do not invent any new experience, dates, achievements, tools, certifications, or metrics.
2. Only use facts explicitly stated in the Source CV or directly reflected by accepted suggestions that remain fact-based.
3. If a missing keyword is not supported by the CV, do not force it into the final documents.
4. Treat ATS diagnostics as optimization guidance, not permission to fabricate content.
5. Apply accepted suggestions actively, not passively.
6. Improve weak areas while preserving strong ones.
7. If there is tension between optimization and truthfulness, truthfulness wins.
8. Do not output JSON.
9. Do not output code fences.
10. Do not explain your reasoning.

==================================================
HOW TO USE THE ATS ANALYSIS
==================================================

Use the ATS information deliberately:
- Use Keyword & Parser Fit to improve terminology alignment and ATS readability.
- Use Semantic Relevance to better align existing experience with role requirements.
- Use Structure & Readability to improve prioritization, clarity, and bullet quality.
- Preserve strong found keywords where relevant.
- Address missing keywords only if they can be supported truthfully by the Source CV.
- Use structure notes to fix weak organization or presentation.

==================================================
HOW TO USE ACCEPTED SUGGESTIONS
==================================================

Accepted suggestions are approved edit instructions.
They are not optional.
You must apply them where appropriate and merge them smoothly into the final result.

If multiple suggestions affect similar content:
...
- The Source CV may be in English or German.
- The Source Job Description may be in English or German.
- You must understand both.
- You must output final documents in BOTH English and German.

The German versions must read like proper professional German application materials.
The English versions must read like polished professional job documents.

==================================================
DOCUMENT OBJECTIVES
==================================================

A) FINAL CV IN ENGLISH
- Tailored to the job description
- ATS-aware
- Recruiter-friendly
- Structured cleanly
- Uses strong, precise, fact-based bullet points
- Highlights the most relevant experiences first
- Improves phrasing, clarity, and relevance using ATS analysis and accepted suggestions

B) FINAL CV IN GERMAN
- Same strategic improvements as the English CV
- Professional German phrasing
- Strong readability and role alignment
- Not a word-for-word translation

C) COVER LETTER IN ENGLISH
- Tailored to the exact job
- Integrates user motivation naturally
- Mentions availability clearly
- Mentions visa/work permit only if provided
- Uses the requested tone
- Connects the candidate’s real background to the role’s needs

D) COVER LETTER IN GERMAN
- Proper German business style
- Same substance as the English version, adapted naturally
...
Output only the four final documents in this exact order:

CV - English

[final tailored CV in English]

CV - German

[final tailored CV in German]

Cover Letter - English

[final tailored cover letter in English]

Cover Letter - German

[final tailored cover letter in German]
"""
    return system_prompt, user_prompt
...
def render_sidebar() -> Dict[str, Any]:
    with st.sidebar:
        ...
        api_key = st.text_input(
            ...
            type="password",
            key="api_key_input",
            help="Use your own provider key. It stays in this session only."
        )

        st.info("Your API key is used only for this session. Do not paste someone else's key.")
